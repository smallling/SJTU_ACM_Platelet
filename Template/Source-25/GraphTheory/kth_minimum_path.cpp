...
int rt[MAXN],seq[MAXN],adj[MAXN],from[MAXN],
  dep[MAXN];
...
int main(){
  while(scanf("%d%d",&n,&m)!=EOF){
    ...
    for(int kth = 1;kth<=Kth;kth++){
      ...
      if(hp[p].lc){
        Q.push(make_pair(
          +hp[hp[p].lc].key+now.first-hp[p].key,
          hp[p].lc));
      }
      if(hp[p].rc){
        Q.push(make_pair(
          +hp[hp[p].rc].key+now.first-hp[p].key,
          hp[p].rc));
      }
      if(rt[edge[hp[p].id].v]){
        Q.push(make_pair(
          hp[rt[edge[hp[p].id].v]].key+now.first,
          rt[edge[hp[p].id].v]));
      }
    }
