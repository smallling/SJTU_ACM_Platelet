...
struct Convex {
  ...
  pair<long long,int>
  get_tangent(vector <Point> &convex,Point vec){
    int l = 0,r = (int)convex.size()-2;
    for(;l+1<r;){
      int mid = (l+r)/2;
      if(sign(
        (convex[mid+1]-convex[mid]).det(vec))>0)
        r = mid;
      else l = mid;
    }
  ...
  void
  update_tangent(const Point &p,int id,int &i0,
                 int &i1){
  ...
  bool contain(Point p){
    if(p.x<lower[0].x||p.x>lower.back().x)
      return false;
    int id =
      lower_bound(lower.begin(),lower.end(),
  ...
  bool get_tangent(Point p,int &i0,int &i1){
    ...
    i0 = i1 = 0;
    int id =
      lower_bound(lower.begin(),lower.end(),p)-
      lower.begin();
  ...
  int get_tangent(Point vec){
    pair<long long,int>
      ret = get_tangent(upper,vec);
    ret.second =
      (ret.second+(int)lower.size()-1)%n;
