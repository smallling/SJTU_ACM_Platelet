...
struct P {
  ...
  bool operator==(P const &that) const{
...
bool in_circumcircle(P const &p1,P const &p2,
                     P const &p3,
                     P const &p4){//p4 in C(p1,p2,p3)
  double u11 = p1.x-p4.x,u21 = p2.x-p4.x,
    u31 = p3.x-p4.x;
  double u12 = p1.y-p4.y,u22 = p2.y-p4.y,
    u32 = p3.y-p4.y;
  double
    u13 = sqr(p1.x)-sqr(p4.x)+sqr(p1.y)-sqr(p4.y);
  double
    u23 = sqr(p2.x)-sqr(p4.x)+sqr(p2.y)-sqr(p4.y);
  double
    u33 = sqr(p3.x)-sqr(p4.x)+sqr(p3.y)-sqr(p4.y);
  double det =
    -u13*u22*u31+u12*u23*u31+u13*u21*u32-
...
struct Edge {
  ...
  SideRef side;
  Edge(): tri(0),side(0){}
  Edge(TriangleRef tri,SideRef side): tri(tri),
                                      side(side){}
};
struct Triangle {
  P p[3];
  ...
  int num_children() const{
    return children[0]==0 ? 0 : children[1]==0 ? 1
                                               :
                                children[2]==0 ? 2
                                               : 3;
  }
...
class Triangulation {
 ...
 private:
  ...
  static TriangleRef
  find(TriangleRef root,P const &p){
    for(;;){
  ...
  void add_point(TriangleRef root,P const &p){
    ...
    tab =
      new(tot_triangles++) Triangle(root->p[0],
                                    root->p[1],p);
    tbc =
      new(tot_triangles++) Triangle(root->p[1],
                                    root->p[2],p);
    tca =
      new(tot_triangles++) Triangle(root->p[2],
                                    root->p[0],p);
  ...
  void flip(TriangleRef tri,SideRef pi){
    ...
    if(!trj||!in_circumcircle(tri->p[0],tri->p[1],
                              tri->p[2],
                              trj->p[pj]))
      return;
    TriangleRef trk =
      new(tot_triangles++) Triangle(
