...
struct Point {
  double x,y;
  Point(){}
  Point(const double &x,const double &y): x(x),
                                          y(y){}
...
int cnt;
inline void psh(const double t1,const Point p1,
...
inline void
combine(int d,const double &area,const Point &o){
  if(sign(area)==0) return;
  centre[d] =
    1/(ans[d]+area)*(ans[d]*centre[d]+area*o);
  ans[d] += area;
}
...
int main(){
  ...
  while(EOF!=scanf("%d%d%d",&m,&n,&index)){
    ...
    fill(centre,centre+n+1,
         Point(0,0));//centre[i]表示上面ans[i]部分的重心
    for(int i(0);i<m;i++)
