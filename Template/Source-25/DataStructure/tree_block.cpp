...
void bfs(){
  ...
  for(int i = n;i;--i){
    ...
    size[son[fa[now]]]<size[now] ? son[fa[now]] =
                                     now : 0;
  }
...
int main(){
  scanf("%d%d",&n,&m);
  for(int i = 1;i<=n;++i)
    scanf("%d",&col[i]),hash[++hcnt] = col[i];
  std::sort(hash+1,hash+hcnt+1);
  hcnt = std::unique(hash+1,hash+hcnt+1)-hash-1;
  for(int i = 1;i<=n;++i)
    col[i] =
      std::lower_bound(hash+1,hash+hcnt+1,col[i])-
      hash;
  for(int i = 1;i<n;++i){
    int a,b;
    scanf("%d%d",&a,&b);
    ...
    if(dep[near[x]]>=dep[lca]){
      ...
      for(;x!=lca;x = fa[x])
        !vis[col[x]] ? vis[jp[++ans] = col[x]] = 1
                     : 0;
      for(;y!=lca;y = fa[y])
        !vis[col[y]] ? vis[jp[++ans] = col[y]] = 1
                     : 0;
      !vis[col[lca]] ? vis[jp[++ans] = col[lca]] =
                         1 : 0;
      for(int i = 1;i<=ans;++i) vis[jp[i]] = 0;
    }
    printf("%d\n",ans);
