...
ll query(Seg *o,int l,int r){
  ...
  if(ql<=mid)
    tmp = query(o->ls,l,mid),cmin(ret,tmp);
  if(mid<qr)
    tmp = query(o->rs,mid+1,r),cmin(ret,tmp);
  return ret;
}
inline void tr_modify(int x,int f){
  ...
  ql = dfn[f];
  qr = dfn[x];
  modify(rt[top[x]],dfn[top[x]],rig[top[x]],qk,
         qb);
}
